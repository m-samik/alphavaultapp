  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="web3" className="flex items-center gap-2">
      ...
      <Card className="bg-vault-dark border-vault-green/40 terminal-border">
        ...
        <CardContent>
          <center className="text-vault-green font-orbitron">Section Coming Soon</center>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {tokens.map(token => (
          <Card key={token.id} className="bg-vault-dark border-vault-green/40 terminal-border group hover:border-vault-green/60 transition-colors">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-sm bg-${token.color}/20 border border-${token.color}/40 flex items-center justify-center`}>
                    <Coins className={`w-5 h-5 text-${token.color}`} />
                  </div>
                  <div>
                    <CardTitle className="text-vault-green font-orbitron">{token.symbol}</CardTitle>
                    <p className="text-xs text-vault-gray-light font-mono">{token.name}</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`flex items-center gap-1 ${token.change24h >= 0 ? 'text-vault-green' : 'text-vault-red'}`}>
                    {token.change24h >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                    <span className="font-mono text-sm">{token.change24h >= 0 ? '+' : ''}{token.change24h}%</span>
                  </div>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-vault-darker border border-vault-green/40 rounded-sm p-3">
                  <p className="text-xs font-mono text-vault-gray-light mb-1">AMOUNT</p>
                  <p className="font-mono text-vault-green font-bold">{formatNumber(token.amount, 6)}</p>
                </div>
                <div className="bg-vault-darker border border-vault-green/40 rounded-sm p-3">
                  <p className="text-xs font-mono text-vault-gray-light mb-1">VALUE</p>
                  <p className="font-mono text-vault-green font-bold">${formatNumber(token.value)}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-vault-darker border border-vault-green/40 rounded-sm p-3">
                  <p className="text-xs font-mono text-vault-gray-light mb-1">PRICE</p>
                  <p className="font-mono text-vault-green">${token.price.toFixed(6)}</p>
                </div>
                <div className="bg-vault-darker border border-vault-green/40 rounded-sm p-3">
                  <p className="text-xs font-mono text-vault-gray-light mb-1">ALLOCATION</p>
                  <div className="flex items-center gap-2">
                    <Percent className="w-3 h-3 text-vault-green" />
                    <span className="font-mono text-vault-green font-bold">{token.percentage}%</span>
                  </div>
                </div>
              </div>
            </CardContent>
